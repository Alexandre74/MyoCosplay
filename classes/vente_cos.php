<?
	class vente_cos {
		...
		function lire_like() {
            ...
            $search = $_POST['search'];
            $search .= '%';
			$requete = "SELECT * FROM vente_cos WHERE nom_perso LIKE'$search'";
			$res = $mysqli->query($requete);
		...
		function aff_block_vente() {
			...
			$requete = "SELECT * FROM vente_cos WHERE nom_perso IN (SELECT nom FROM personnages WHERE id = $end)";
			$res = $mysqli->query($requete);
			
			while($row = $res->fetch_array(MYSQLI_ASSOC)){		
				$id = $row["id"];
					
                $requete2 =  "SELECT * FROM vente_cos WHERE id = '$id' ORDER BY date_crea DESC";
                $res2 = $mysqli->query($requete2);
                $row2 = $res2->fetch_array(MYSQLI_ASSOC);
