<?
	class evenement {
		...
		function lire_all() {
			...
			$requete = "SELECT id FROM evenements ORDER BY id ";
			$res = $mysqli->query($requete);
		
			while($row = $res->fetch_array(MYSQLI_ASSOC)){		
				$id = $row["id"];
				
				$requete2 =  "SELECT * FROM evenements WHERE id = '$id' ";
				...
				if ($x1 != ""){
					...
					$x3 = htmlspecialchars($row2[$_COOKIE["lang"]."_youtube"]);
					...
					$x3 = htmlspecialchars($row2["FR_youtube"]);
		...
		function lire() {
			...
			$requete = "SELECT * FROM evenements WHERE id='$end'";
			$res = $mysqli->query($requete);
		...
		function recup_event() {
			...
			$requete = "SELECT * FROM evenements ORDER BY date DESC LIMIT 3";
			$res = $mysqli->query($requete);
			...
			while($row = $res->fetch_array(MYSQLI_ASSOC)){		
				...
				$retour[$flag]["event_id"] 	= $row["id"];

				$x1 = htmlspecialchars($row[$_COOKIE["lang"]."_titre"]);
				if ($x1 != ""){
				//if (isset(htmlspecialchars($row[$_COOKIE["lang"]."_titre"]))){
					$x1 = htmlspecialchars($row[$_COOKIE["lang"]."_titre"]); 
					$x2 = htmlspecialchars($row[$_COOKIE["lang"]."_cover_img"]);
					$x3 = htmlspecialchars($row[$_COOKIE["lang"]."_youtube"]);
					$x4 = htmlspecialchars($row[$_COOKIE["lang"]."_contenu"]);
